 ...
 */
contract XToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint) {
         ...
         *  On success, the XToken holds an additional mintAmount of cash.
     ...
     * @param redeemTokensIn The number of XTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @param redeemAmountIn The number of XTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, XToken XTokenCollateral) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, XToken XTokenCollateral) internal returns (uint) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(XTokenCollateral), repayAmount);
        ...
        if (seizeTokens > XTokenCollateral.balanceOf(borrower)) {
        ...
        uint seizeError = XTokenCollateral.seize(liquidator, borrower, seizeTokens);
        require(seizeError == uint(Error.NO_ERROR), "token seizure failed");

        /* We emit a LiquidateBorrow event */
        emit LiquidateBorrow(liquidator, borrower, repayAmount, address(XTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(XTokenCollateral), liquidator, borrower, repayAmount, seizeTokens);
